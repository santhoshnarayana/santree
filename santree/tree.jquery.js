  ...
  */
  ...
  Node = (function() {
    ...
    */
    ...
    Node.prototype.getData = function() {
      ...
      getDataFromNodes = function(nodes) {
        ...
        for (_i = 0, _len = nodes.length; _i < _len; _i++) {
          ...
          for (k in node) {
            v = node[k];
            if ((k !== 'parent' && k !== 'children' && k !== 'element' && k !== 'tree' && k!=='is_open'&&k!=='licenceCount') && Object.prototype.hasOwnProperty.call(node, k)) {
               // console.log('Print ',k);
              tmp_node[k] = v;
            }
          }
	...
	//Added by 138 for getting the full details of the tree;
	
	Node.prototype.getFullData = function () {
            var getChildrenDataFromNodes, getParentDataFromNodes;


            getChildrenDataFromNodes = function (nodes) {
                var data, k, node, tmp_node, v, _i, _len;
                ...
                for (_i = 0, _len = nodes.length; _i < _len; _i++) {
                    ...
                    for (k in node) {
                        ...
                        if ((k !== 'parent' && k !== 'children' && k !== 'element' && k !== 'tree' && k!=='licenceCount') && Object.prototype.hasOwnProperty.call(node, k)) {

                            tmp_node[k] = v;
                        }
                    }
                    if (node.hasChildren()) {
                        tmp_node.children = getChildrenDataFromNodes(node.children);
                    }
                    data.push(tmp_node);
                }
                return data;
            ...
            getParentDataFromNodes = function (node, inherit) {
                var data = { };
                ...
                if (inherit !== '') {
                    data.children = inherit;

                } else {

                    if (node.hasChildren())
                        data.children = getChildrenDataFromNodes(node.children);
                }

                if (node.parent && node.parent.parent) {
                    data = getParentDataFromNodes(node.parent, new Array(data));
                } else {
                    data = new Array(data);
                }
            ...
            return getParentDataFromNodes(this, '');
        };


	Node.prototype.getDataWithSelf = function () {
            var getDataFromNodes, data = {};
            ...
            for (k in this) {
                v = this[k];
                if ((k !== 'parent' && k !== 'children' && k !== 'element' && k !== 'tree' && k!=='licenceCount') && Object.prototype.hasOwnProperty.call(this, k)) {
                    data[k] = v;
                }
            }
            data.children = getDataFromNodes(this.children);
       // console.log('Santhosh  Siva');
       // console.dir(data);
            return new Array(data);
        };

	//code ends for getFullData() by 138
  ...
  */
  ...
  JqTreeWidget = (function(_super) {
    ...
    JqTreeWidget.prototype._loadDataFromUrl = function(url_info, parent_node, on_finished) {
      ...
      return $.ajax({
        ...
        success: function(response) {
          ...
          removeLoadingClass();
          _this._loadData(data, aparent_node);
    ...
    JqTreeWidget.prototype._refreshElements = function(from_node) {
	  ...
	  var img='<img src="images/error-icon.png" class="expiredImg" id="module_SelectedExpired"/>';
	   $('.expiredImg').prop('title', 'The license for this module has expired');
       $('.expiredImg').tooltip({
            track: true
        });
		...
		if(node.licenceCount!=null){		
        escaped_name = escapeIfNecessary(node.name) + '('+node.licenceCount+')';
		if(node.licenceCount===-1){	
		escaped_name = escapeIfNecessary(node.name) +"  "+img;
		}
		}		
		else
		{
      ...
      createFolderLi = function(node) {
        ...
        folder_classes = getFolderClasses();
		 var img='<img src="images/error-icon.png" class="expiredImg"/>';
          if(node.licenceCount!=null)
			{
              escaped_name = escapeIfNecessary(node.name) + '('+node.licenceCount+')';
			if(node.licenceCount===-1){
  ...
  NodeElement = (function() {
	...
	//adding the user image to the selected modules @74
	$('#module_Selected').remove();
	 var img='<img src="images/User_Icon.png" class="moduleuserimg" id="module_Selected"/>';
	 var id = this.node.id;
	 
	 if(null!=this.node.licenceCount)
	 {
	 this.getSpan().append(img);
	 }
	 
	 $('#module_Selected').on('click',function(){
			getAllDetailsAttachedToModule(id);
			$('#cancelUserIconId').off('click');
				});
